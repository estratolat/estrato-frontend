      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            title="Catálogos, carga, análisis y mapa"
          >
      ...
      <div className="space-y-6">
        {!eleccionId ? (
          <div className="card p-8 text-center">
            ...
            <p className="mx-auto mb-6 max-w-xl text-sm text-secondary-600">
              Selecciona una elección arriba para comenzar. La IA detectará
              automáticamente tus datos y te sugerirá el actor principal, el
              rival a vencer y el territorio a analizar.
            </p>
            {elecciones.length === 0 && (
              <button
                onClick={() => {
              ...
              >
                <Plus size={16} /> Crear mi primera elección
              </button>
            )}
          </div>
        ) : (
            ...
            {cargandoEstado ? (
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {[
                  {
                    key: "sábanas",
                    label: "Sábanas cargadas",
                    icon: FileSpreadsheet,
                  },
                  {
                    key: "históricos",
                    label: "Históricos electorales",
                    icon: BarChart3,
                  },
                  ...
                  return (
                    ...
                    >
                      <item.icon
                        size={16}
                        className={
                          tiene ? "text-primary-600" : "text-secondary-300"
                        }
                      />
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                {estadoInteligencia.sugerencias?.rival?.partido && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-2 py-1 text-xs font-medium text-red-700">
                    ...
                    {estadoInteligencia.sugerencias.rival.nombre ||
                      estadoInteligencia.sugerencias.rival.partido}
                  </span>
                )}
            ...
            {!cargandoEstado && estadoInteligencia && (
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <button
                  ...
                  className="btn-secondary flex items-center gap-2 text-xs"
                >
                  <BrainCircuit size={14} /> Ver análisis por sección
                </button>
                <button
                  onClick={() => {
                    setMostrarConfigAvanzada(true);
                  ...
                  className="btn-secondary flex items-center gap-2 text-xs"
                >
                  <MapPin size={14} /> Ver mapa territorial
                </button>
              </div>
            )}
          </div>
        ...
        <div className="card p-6">
          ...
          <p className="mb-4 text-sm text-secondary-600">
            Escribe lo que quieres saber o decidir. La IA cruza proyección,
            histórico, votantes, líderes, eventos, encuestas, sedes, monitoreo
            de casillas, perfil del candidato, indicadores municipales de Data y
            la perspectiva del actor principal. Selecciona abajo qué datos
            incluir.
          </p>
          ...
          <div className="mb-4 grid gap-4 lg:grid-cols-3">
            <div className="space-y-3 lg:col-span-1">
              <div className="space-y-2">
                <label className="label flex items-center gap-2">
                  <Users size={16} className="text-primary-600" /> Actor
                  principal / perspectiva del análisis
                </label>
                <select
                ...
                >
                  ...
                  {actores.map((a) => (
                    <option key={a.id} value={a.id}>
                      {a.nombre_visual}{" "}
                      {a.partido?.siglas ? `(${a.partido.siglas})` : ""}
                    </option>
                  ))}
                </select>
                <p className="text-xs text-secondary-500">
                  La IA analizará la estrategia desde la perspectiva de este
                  actor.
                </p>
              </div>
              ...
              {fuentesIA.historico && historicosDisponibles.length > 0 && (
                <div className="space-y-2">
                  ...
                  >
                    <option value="">
                      Usar todos los históricos disponibles
                    </option>
                    {historicosDisponibles.map((h) => {
                      const key = `${h.tipo_historico}|${h.tipo_eleccion}|${h.anio}|${h.estado_id || ""}|${h.municipio_id || ""}`;
                  ...
                  <p className="text-xs text-secondary-500">
                    Selecciona un histórico específico para que la IA use solo
                    esos registros y secciones.
                  </p>
                </div>
              )}
              ...
              {fuentesIA.historico && rivalesDisponibles.length > 0 && (
                <div className="space-y-2">
                  <label className="label flex items-center gap-2">
                    <Target size={16} className="text-primary-600" /> Rival
                    principal a vencer
                  </label>
                  <select
                    value={rival.partido || ""}
                    onChange={(e) => {
                      ...
                      const r = rivalesDisponibles.find(
                        (x) => x.partido === partido,
                      );
                      setRival({
                        partido: r?.partido,
                  ...
                  >
                    ...
                    {rivalesDisponibles.map((r) => (
                      <option key={r.partido} value={r.partido}>
                        {r.partido} {r.candidato ? `(${r.candidato})` : ""} —{" "}
                        {r.votos.toLocaleString()} votos
                      </option>
                    ))}
                  </select>
                  <p className="text-xs text-secondary-500">
                    Tomado del histórico electoral. La IA enfocará la estrategia
                    para vencer a este actor.
                  </p>
                </div>
              )}

              <label className="label flex items-center gap-2">
                <Database size={16} className="text-primary-600" /> Fuentes de
                datos a vincular
              </label>
              ...
              <div className="mt-3 space-y-2">
                <label className="label flex items-center gap-2">
                  <MapPin size={16} className="text-primary-600" /> Filtro
                  territorial
                </label>
                <select
                  ...
                  <div className="rounded-lg border border-gray-200 bg-white p-2">
                    <div className="mb-2 flex items-center justify-between">
                      <p className="text-xs font-medium text-secondary-700">
                        Selecciona las secciones a analizar
                      </p>
                      <button
                        type="button"
                        onClick={() => {
                          const todas = secciones.map((s) => s.seccion);
                          const actuales = filtroTerritorialIA.valor
                            .split(",")
                            .map((v) => v.trim())
                            .filter(Boolean);
                          const todasSeleccionadas =
                            actuales.length === todas.length &&
                            todas.every((t) => actuales.includes(t));
                          setFiltroTerritorialIA({
                            ...filtroTerritorialIA,
                      ...
                      >
                        {filtroTerritorialIA.valor.split(",").filter(Boolean)
                    ...
                    <div className="max-h-40 overflow-y-auto">
                      <div className="grid grid-cols-3 gap-2">
                        {secciones.map((s) => {
                          const seleccionadas = filtroTerritorialIA.valor
                            .split(",")
                            .map((v) => v.trim())
                            .filter(Boolean);
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const nuevas = e.target.checked
                                    ? [...seleccionadas, s.seccion].filter(
                ...
                {filtroTerritorialIA.tipo === "municipio" && (
                  <div className="space-y-2">
                    ...
                    {historicosDisponibles.length > 0 && (
                      <p className="text-xs text-secondary-500">
                        Tip: si seleccionas un histórico electoral arriba, la IA
                        usará automáticamente las secciones de ese municipio.
                      </p>
                    )}
                  </div>
                )}
            ...
            <div className="space-y-3 lg:col-span-1">
              <label className="label">
                Describe el contexto de tu campaña (formulario libre)
              </label>
              {[
                {
                  key: "objetivo",
                  label: "Objetivo electoral",
                  placeholder:
                    "Ej. Ganar la gubernatura con 45% de votos válidos",
                },
                {
                  key: "escenario",
                  label: "Escenario / rivalidad",
                  placeholder:
                    "Ej. Competencia tripartita, el PRI lleva 12 años gobernando",
                },
                {
                  key: "preocupaciones",
                  label: "Preocupaciones",
                  placeholder:
                    "Ej. Baja participación en zonas rurales y votos nulos altos",
                },
                {
                  key: "oportunidades",
                  label: "Oportunidades",
                  placeholder:
                    "Ej. Fuerte crecimiento de jóvenes votantes en zona metropolitana",
                },
                {
                  key: "instrucciones",
                  label: "Instrucciones especiales",
                  placeholder:
                    "Ej. Enfócate en defender bastiones y recuperar secciones pérdidas en 2021",
                },
              ].map((campo) => (
                <div key={campo.key}>
                  <p className="mb-1 text-xs font-medium text-secondary-700">
                    {campo.label}
                  </p>
                  <textarea
            ...
            <div className="flex flex-col gap-3 lg:col-span-1">
              <label className="label">
                Tu pregunta o instrucción para la IA
              </label>
                ...
                onClick={async () => {
                  ...
                  try {
                    const { data } = await inteligenciaElectoralApi.consultarIA(
                      {
                        ...
                        historicoSeleccion: Object.keys(historicoSeleccion)
                          .length
                          ? historicoSeleccion
                          : undefined,
                    ...
                    if (eleccionId) {
                      ...
                      localStorage.setItem(
                        claveHistorialIA(eleccionId),
                        JSON.stringify(actualizado.slice(0, 50)),
                      );
                    }
                  } catch (err: any) {
                    setError(
                      err.response?.data?.message || "Error al consultar la IA",
                    );
                  } finally {
          ...
          {respuestaIA && (
            <div className="mt-6 rounded-lg border border-primary-200 bg-primary-50 p-4">
              <h4 className="mb-2 flex items-center gap-2 text-sm font-bold text-secondary-900">
                <Sparkles size={16} className="text-primary-600" /> Respuesta de
                la IA
              </h4>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              {resumenContextoIA?.filtro_territorial?.tipo !== "todos" && (
                <span className="inline-flex rounded-full bg-primary-50 px-2 py-1 text-xs font-medium text-primary-700">
                  Filtro: {resumenContextoIA?.filtro_territorial?.tipo} ·{" "}
                  {resumenContextoIA?.filtro_territorial?.valor}
                </span>
              )}
            </div>
          )}
          ...
          {historialIA.length > 0 && (
            <div className="mt-6 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    if (
                      confirm(
                        "¿Borrar todo el historial de análisis de esta elección?",
                      )
                    ) {
                      setHistorialIA([]);
                      setHistorialExpandidoId(null);
                      if (eleccionId)
                        localStorage.removeItem(claveHistorialIA(eleccionId));
                    }
                  }}
              ...
              {historialIA.map((item, idx) => {
                ...
                const expandido =
                  historialExpandidoId === item.id ||
                  (historialExpandidoId === null && esUltimo);
                return (
                  <div
                    key={item.id}
                    className={`card overflow-hidden ${esUltimo ? "border-primary-300" : "border-gray-200"}`}
                  >
                    <button
                      onClick={() =>
                        setHistorialExpandidoId(expandido ? null : item.id)
                      }
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-secondary-500">
                          {expandido ? "Ocultar" : "Ver respuesta"}
                        </span>
                        <svg
                      ...
                      <div className="border-t border-gray-100 p-4">
                        {item.contexto_resumen && (
                          <div className="mb-4 flex flex-wrap gap-2">
                            ...
                            {item.contexto_resumen?.filtro_territorial?.tipo !==
                              "todos" && (
                              <span className="inline-flex rounded-full bg-primary-50 px-2 py-1 text-xs font-medium text-primary-700">
                                Filtro:{" "}
                                {
                                  item.contexto_resumen?.filtro_territorial
                                    ?.tipo
                                }{" "}
                                ·{" "}
                                {
                        ...
                        <div className="mt-4 flex justify-end">
                          <button
                            onClick={() =>
                              eliminarDelHistorial(eleccionId || "", item.id)
                            }
                            className="text-xs font-medium text-red-600 hover:text-red-700"
                          >
              ...
              {[
                {
                  key: "catalogos",
                  label: "Catálogos",
                  desc: "Crea partidos, elecciones y actores.",
                  icon: Users,
                  color: "bg-blue-50 text-blue-700 border-blue-200",
                },
                {
                  key: "carga",
                  label: "Cargar sábanas",
                  desc: "Sube el Excel con resultados por casilla.",
                  icon: Upload,
                  color: "bg-green-50 text-green-700 border-green-200",
                },
                {
                  key: "analisis",
                  label: "Análisis por sección",
                  desc: "Revisa secciones, ganador, votos y clasificación.",
                  icon: BrainCircuit,
                ...
                },
                {
                  ...
                  desc: "Visualiza las secciones coloreadas por ganador.",
                  icon: MapPin,
                ...
                <button
                  ...
                  className={`flex items-start gap-4 rounded-xl border p-4 text-left transition hover:shadow-md ${t.color}`}
                >
                  <div className="rounded-lg bg-white p-2 shadow-sm">
                    <t.icon size={24} className="text-secondary-700" />
                  </div>
                  <div>
                    <p className="font-bold text-secondary-900">{t.label}</p>
                    <p className="text-sm text-secondary-600">{t.desc}</p>
                  </div>
                  <ChevronRight
                    size={20}
                    className="ml-auto text-secondary-400"
                  />
                </button>
              ))}
            </div>
